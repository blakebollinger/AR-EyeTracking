...
Based of the HMD Eyes project, this will start a notification server (on another thread).
Then, it will initiate a calibration squence.

'''

'''
Known Issues arise when using the wrong version of msgpack
If you recieve an error similar to "TypeError: unpackb() got an unexpected keyword argument 'encoding'"
Uninstall 'msgpack' and install 'msgpack-python'
If error persists try installing 'msgpack-python' and uninstalling 'msgpack'
'''
...
print("Waiting for Notification server to start")
time.sleep(3)

text = input("Please start Pupil Capture...Press any key when running")
